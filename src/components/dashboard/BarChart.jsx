  ...
  <g className='legend' transform={`translate(${size[0]}, 50)`}>
    <g className='legendCells'>
      {["Wave 1", "Wave 2", "Wave 3", "Wave 4"].map((text, i) => (
        <g key={i} className='cell' transform={`translate(0, ${i*20})`}>
          <rect className='swatch' height='15' width='15' fill={legendColors[i]} />
          <text className='label' transform={`translate(25, 12.5)`}>{text}</text>
        </g>
        ))}
    </g>
  </g>
...
const bars = data.map((d, i) => {
  return <rect key={i} className='bar' onMouseOver={onHover} 
    x={i*barWidth} y={size[1] - (yScale(d.data[i] ?? 0))}
