...
const ptOps = {
    ...
    anchor: (p1) =>
        Math.abs(p1.x) < 0.2 * HEIGHT && Math.abs(p1.y) < 0.2 * HEIGHT
            ? { x: 0, y: 0 }
            : p1,
    cap: (p1) => ({
        x: Math.sign(p1.x) * Math.min(Math.abs(p1.x), WIDTH),
        y: Math.sign(p1.y) * Math.min(Math.abs(p1.y), HEIGHT),
    }),
    max: (p1) =>
        Math.abs(p1.x) > Math.abs(p1.y) ? { x: p1.x, y: 0 } : { x: 0, y: p1.y },
    scale: (p1, factor) => ({ x: p1.x / factor, y: p1.y / factor }),
    translate: (p1, factor) => ({ x: p1.x + factor, y: p1.y + factor }),
}
const infixPts = (p1, operator, p2 = { x: 0, y: 0 }) =>
    ptOps[operator]?.(p1, p2) ??
    console.error('Point Operation not found: ' + operator)
...
export const ZoomPan = () => {
    ...
    useLayoutEffect(() => {
        ...
        if (center === null) {
            ...
            const scaleFocal = infixPts(
                freeZoom ? anchored : maxOrd,
                'scale',
                scale,
            )
            // adjust = infixPts(adjust, '+', scaleFocal)
            adjust = infixPts(
                infixPts(adjust, '+', freeZoom ? scaleFocal : maxOrd),
                'cap',
            )
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        backgroundPosition: `${-adjust.x - offset.x}px ${
                            -adjust.y - offset.y
                        }px`,
                    }}
                >
                ...
                <Hud>
                    Scale: {scale.toFixed(1)} {interaction ?? ''}
                </Hud>
                <Hud>
                    Inset:{' '}
                    {`{x: ${inset.x.toFixed(0)}, y: ${inset.y.toFixed(0)}}`}
                </Hud>
                <Hud>Offset: {`{x: ${offset.x}, y: ${offset.y}}`}</Hud>
                <Hud>
                    Adjustment:{' '}
                    {`{x: ${adjust.x.toFixed(0)}, y: ${adjust.y.toFixed(0)}}`}
                </Hud>
                <Hud>
                    Mouse:{' '}
                    {`{x: ${(mousePos.x - (center?.x ?? 0)).toFixed(0)}, y: ${(
                        mousePos.y - (center?.y ?? 0)
                    ).toFixed(0)}}`}
                </Hud>
            </div>
            <button onClick={() => setFreeZoom(!freeZoom)}>
